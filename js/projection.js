/*jslint browser: true*/
/*global d3: true, $: true, identity: true, lookup: true*/
"use strict";
...
$(document).ready(function () {
	var svg = d3.select("#data")
		...
		mapGroup = svg.append("svg:g")
		.attr("id", "map"),

		circleGroup = svg.append("svg:g")
		.attr("id", "circles");
	...
	d3.csv("Malawi_Digested.csv", function (response) {
		...
		var projects = response.map(project_object),
			plot_projects = function (projects_to_plot) {
				// Add the circles.
		...
		function generate_filters(filter_key) {
			var group_names = projects.map(function (object) { return object[filter_key]; }).sort_unique(),
				filterDivs = d3.select("#filters").selectAll("div")
